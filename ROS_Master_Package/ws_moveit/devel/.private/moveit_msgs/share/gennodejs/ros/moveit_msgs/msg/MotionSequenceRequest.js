...
class MotionSequenceRequest {
  ...
  static serialize(obj, buffer, bufferOffset) {
    // Serializes a message object of type MotionSequenceRequest
    ...
    obj.items.forEach((val) => {
      bufferOffset = MotionSequenceItem.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }

  static deserialize(buffer, bufferOffset=[0]) {
    //deserializes a message object of type MotionSequenceRequest
    let len;
    let data = new MotionSequenceRequest(null);
    // Deserialize message field [items]
    // Deserialize array length for message field [items]
    len = _deserializer.uint32(buffer, bufferOffset);
    data.items = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.items[i] = MotionSequenceItem.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.items.forEach((val) => {
      length += MotionSequenceItem.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static datatype() {
    ...
    return 'moveit_msgs/MotionSequenceRequest';
  }

  static md5sum() {
    //Returns md5sum for a message object
    return 'd6e6d4da3bede363f50e4ffbac7629a0';
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new MotionSequenceRequest(null);
    if (msg.items !== undefined) {
      resolved.items = new Array(msg.items.length);
      for (let i = 0; i < resolved.items.length; ++i) {
        resolved.items[i] = MotionSequenceItem.Resolve(msg.items[i]);
      }
    }
    else {
      resolved.items = []
...
module.exports = MotionSequenceRequest;
